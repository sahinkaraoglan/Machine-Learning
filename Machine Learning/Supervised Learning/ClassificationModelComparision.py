...
from sklearn.inspection import DecisionBounderDisplay
...
for ds_cnt, ds in enumerate(datasets):
    ...
    ax.scatter(X[:,0],X[0:,1], c = y, cmap = plt.cm.coolwarm, edgecolors = "black")
    i+= 1
    
plt.show()
    
names = ["Nearest Neighbors", "Linear SVM", "Decision Tree", "Random Forest", "Navie Bayes"]
...
for ds_cnt, ds in enumerate(datasets):
    ...
    #plot training data
    ax.scatter(X_train[:,0], X[:,1], c = y_train, cmap = cm_bright, edgecolors = "black")
    
    
    #plot test data
    ax.scatter(X_train[:,0], X[:,1], c = y_train, cmap = cm_bright, edgecolors = "black", alpha = 0.6)
    ...
    for name, clf in zip(names, classifiers):
        ...
        DecisionBounderDisplay.from_estimator(clf, X, cmap = plt.cm.RdBu, alpha = 0.7, ax = ax, eps = 0.5)
